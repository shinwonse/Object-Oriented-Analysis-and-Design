...
class DVM4Test {
    ...
    void getDrinkListTest(){
        initDrinkList();
        DVM4 dvm = new DVM4(drinkArrayList4, 4, 404);
        ArrayList<Drink> drink_list = dvm.getDrink_list();
        assertEquals(11, drink_list.get(0).getStock());
        assertEquals(4, drink_list.get(1).getStock());
        assertEquals(8, drink_list.get(2).getStock());
        assertEquals(10, drink_list.get(3).getStock());
        assertEquals(10, drink_list.get(4).getStock());
        assertEquals(4, drink_list.get(5).getStock());
        assertEquals(21, drink_list.get(6).getStock());
    ...
    void updateStockTest(){
        Drink drink = new Drink("코카콜라", 1500, 10, "src/main/resources/image/1.jpg");
        drinkArrayList4.add(drink);
        DVM3 dvm = new DVM3(drinkArrayList4, 4, 404);
...
//    }
    ...
    void run_StockResponseTest() throws IOException {
        ...
        Message message = new Message();
        message.createMessage(SOURCE_ID, 4, MsgType.REQUEST_STOCK, "펩시콜라");
        dummySocket.sendSocket(socket, message);
        Message message2 = new Message();
        message2.createMessage(SOURCE_ID, 4, 999);
    ...
    void  run_ResponseLocationTest() throws IOException {
        ...
        DummySocket dummySocket = new DummySocket();
        Message message = new Message();
        message.createMessage(SOURCE_ID, 4, MsgType.REQUEST_LOCATION);
        dummySocket.sendSocket(socket, message);
        Message message2 = new Message();
        message2.createMessage(SOURCE_ID, 4, 999);
        dummySocket.sendSocket(socket2, message2);
        ...
        assertEquals("404", receivedMessage.getMsg());
    ...
    void run_ResponseSaleCheckTest() throws IOException {
        ...
        DummySocket dummySocket = new DummySocket();
        Message message = new Message();
        message.createMessage(SOURCE_ID, 4, MsgType.DRINK_SALE_CHECK, "펩시콜라");
        dummySocket.sendSocket(socket, message);
        Message message2 = new Message();
        message2.createMessage(SOURCE_ID, 4, 999);
        dummySocket.sendSocket(socket2, message2);
